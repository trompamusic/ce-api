...
export const queryResolvers = {
  Query: {
    ...
    searchMetadataText (object, params, context, resolveInfo) {
      ...
      const doEvaluateTypeSubset = !(!(params.onTypes instanceof Array) || params.onTypes.length === 0 || params.onTypes.length === resolveInfo.schema._typeMap.MetadataInterfaceType._values.length)
      // determine whether to evaluate only a subset of Metadata fields
      const doEvaluateFieldSubset = !(!(params.onFields instanceof Array) || params.onFields.length === 0 || params.onFields.length === resolveInfo.schema._typeMap.SearchableMetadataFields._values.length)
